...
{/* In this example, the `signUp` method registers a new user with the username `chiro-user` and email `chiro@example.com`. The signer must be setup beforehand to authenticate the sign-up process see [Setup your credentials](../getting-started.mdx#setup-your-credentials). */}
