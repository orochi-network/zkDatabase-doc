...
```ts
const zkStatus = (await zkdb.db(databaseName).zkProofState()).unwrap();
```
...
// Get zkProof state
const zkStatus = (await dbTest.zkProofState()).unwrap();
console.log('ZK Proof state:', zkStatus);`}
/>
...
:::info
The `Failed` status also indicates that subsequent operations to generate
proofs will be blocked until corrective action is taken. Refer to [Detect and
retry failed operations](./prover-retry.mdx) for more information on how to
handle failure.
:::
