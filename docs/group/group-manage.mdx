...
```ts
await zkdb.db(databaseName: string).group('group-name').userAdd(listUser: string[]);
```
...
```ts
const result = await zkdb.db('my-db')
  .group('group-name')
...
```ts
await zkdb.db(databaseName: string).group(groupName: string).userRemove(listUser: string[]);
```
